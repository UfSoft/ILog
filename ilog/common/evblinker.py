...
#        with_stack = kwargs.pop('_withstack', False)
...
#            sender_name = sender
#
#        log.log(5, "signal: %r  sender: %r  kwargs: %r  waitall: %s",
#                self.name, sender_name, kwargs, waitall)
#
#
#
...
#                log.log(5, "Grabing results from receiver: %s", receiver)
#                try:
...
#
#
#NamedSignal = blinker.base.NamedSignal
    ...
    def send(self, *sender, **kwargs):
        ...
        for receiver in self.receivers_for(sender):
            ...
            try:
                ...
                log.exception(err)
            try:
                # Allow other green code to run
                eventlet.sleep(0)
            except AssertionError, err:
                if str(err) != 'do not call blocking functions from the mainloop':
                    log.exception(err)
